...
const CONTENT_SCRIPT_BUILD = "0.5.78";
  ...
  try {
    ...
    switch (message.type) {
      ...
      case "dump_conversation":
        await handleDumpConversation(message);
        break;
...
function modelSelectionFailureDiagnostics(selection) {
  ...
  for (const key of [
    ...
    "post_close_picker_close_verification",
    "post_close_closed_pill_family_status",
    "post_close_closed_pill_effort_status",
    "family_status",
    "effort_status",
...
// ownership model, same tab query + owned-tab filter) but forwards a read-only
// capture command to the resolved tab — it serializes the conversation
// container and never clicks, types, or navigates — and relays the reply
// (HTML + extractor-vs-raw text lengths) as a terminal job_complete.
async function handleDumpConversation(message) {
  ...
  if (targetedJob) {
    ...
    if (runId !== targetedJob.run_id) {
      throw commandError(
        "run_mismatch",
        `dump_conversation target run ${runId} does not match active job ${targetedJob.job_id}`,
        {
          phase: "profile",
          side_effect_started: false,
          expected_run_id: targetedJob.run_id,
  ...
  const tabs = await chrome.tabs.query({ url: adapter.tabQueryPattern });
  // allow_live_job is threaded from the CLI payload so the content-script
  // gate (dumpConversation) sees it; without this the gate is inert and the
  // flag is silently ignored.
  const allowLiveJob = message.payload?.allow_live_job === true;
  let captured = null;
  const errors = [];
  for (const tab of tabs) {
    ...
    try {
      ...
      const isRunMismatch = error?.code === "run_mismatch";
      // Same policy as dump_picker_html: a content-script exception on an
      // ownership-matched tab surfaces as that tab's own code and message.
      if (!isRunMismatch && error?.code) {
        await postTerminalMessage(
          message,
          errorEnvelope(messageJob(message), error.code, String(error?.message ?? error), {
