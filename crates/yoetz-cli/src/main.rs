...
struct BrowserExtensionInspectArgs {
    ...
    /// capture diagnostics (bytes, opened_by_us) on stderr.
...
async fn run_recipe_via_chrome_devtools_mcp(
    ...
    let payload = chatgpt_recipe::ChatgptRecipeOutput {
        ...
        final_model_selection: response.final_model_selection.clone(),
        warnings: Vec::new(),
        fallback_used,
    ...
    match format {
        ...
        OutputFormat::Jsonl => {
            let event = chatgpt_recipe::ChatgptRecipeOutput {
                ...
                model_selection_status,
                final_model_selection: payload.get("final_model_selection").cloned(),
...
fn build_chatgpt_recipe_spec(
    ...
    ensure_chatgpt_sol_chat_pro_only_vars(recipe_vars)?;
...
fn ensure_chatgpt_sol_chat_pro_only_vars(recipe_vars: &BTreeMap<String, String>) -> Result<()> {
    ...
    let (payload, jsonl_event, notification_target, prepared_thread) = match builtin_recipe {
        web_recipe::BuiltinWebRecipe::Chatgpt => {
            ...
            let output = chatgpt_recipe::ChatgptRecipeOutput {
                ...
                model_used: response.model_used,
                model_selection_status: response.model_selection_status,
                final_model_selection: response.final_model_selection,
...
mod tests {
    ...
    #[test]
    fn builtin_chatgpt_recipe_defaults_flow_into_shared_sol_chat_pro_spec() {
